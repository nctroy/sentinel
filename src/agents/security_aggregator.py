...
class SecurityAggregatorAgent(SubAgent):
    ...
    async def diagnose(self) -> Dict[str, Any]:
        ...
        if summary["total_findings"] > 0:
            ...
            description = (
                f"Found {summary['total_findings']} security vulnerabilities "
            ...
            if critical_count > 0:
                impact = 10.0
                description = (
                    f"🚨 CRITICAL SECURITY ALERT: {critical_count} critical "
                    "vulnerabilities detected. Immediate action required."
                )
                recommended_action = (
                    "STOP THE LINE. Fix critical vulnerabilities immediately."
                )
            else:
                impact = min(9.0, (high_count * 2.0) + 1.0)
                recommended_action = (
                    "Review and triage security findings in the Command Center."
                )
                ...
                for result in run.get("results", []):
                    ...
                    vulnerability = {
                        ...
                        "remediation": rule.get(
                            "helpUri", "Check tool documentation for remediation"
                        ),
                        "raw_data": result,
                    }
