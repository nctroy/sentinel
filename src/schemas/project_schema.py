...
class ProjectConfig(BaseModel):
    ...
    outcome_metrics: Dict[str, str] = Field(..., description="Measurable success metrics")
    definition_of_done: List[str] = Field(..., description="Acceptance criteria")
    milestones: List[ProjectMilestone] = Field(..., max_items=7, description="3-7 milestones max")
    risks: List[ProjectRisk] = Field(..., max_items=3, description="Top 3 risks + mitigations")
